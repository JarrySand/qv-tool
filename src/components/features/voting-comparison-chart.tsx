...
export function VotingComparisonChart({
  ...
  const { singleVoteResults, hiddenVotes, totalHiddenVotes, totalQvVotes } = hiddenPreferences;
  
  // QV結果をソート
  const sortedQvResults = [...qvResults].sort((a, b) => b.totalVotes - a.totalVotes);
  ...
  const hiddenPercentage = totalQvVotes > 0 ? (totalHiddenVotes / totalQvVotes) * 100 : 0;
  ...
  return (
    <Card className="overflow-hidden">
      ...
      <CardContent className="pt-6">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <div>
            <div className="mb-4 flex items-center justify-between">
              <h3 className="font-semibold text-muted-foreground">一人一票の場合</h3>
          ...
          <div>
            <div className="mb-4 flex items-center justify-between">
              <h3 className="font-semibold text-secondary">二次投票の結果</h3>
              <span className="text-sm text-muted-foreground">
                総票数: {totalQvVotes}
              </span>
            </div>
                ...
                <div key={result.id}>
                  ...
                  <div className="h-3 overflow-hidden rounded-full bg-muted">
                    <div
                      ...
                      style={{ width: `${maxVotes > 0 ? (result.totalVotes / maxVotes) * 100 : 0}%` }}
                    />
                  </div>
                </div>
              ))}
        ...
        {/* 埋もれた選好 */}
        {totalHiddenVotes > 0 && (
          <div className="mt-6 rounded-xl bg-gradient-to-r from-secondary/10 to-accent/10 p-4">
            ...
            <p className="mt-3 text-sm text-muted-foreground">
              ...
              二次投票ではこの {hiddenPercentage.toFixed(1)}% の選好も反映されています。
            </p>
          </div>
        )}
          ...
          <div className="mt-6">
            ...
            <div className="space-y-2">
              {hiddenVotes.slice(0, 5).map(subject => {
                const qvResult = qvResults.find(r => r.id === subject.subjectId);
                const singleResult = singleVoteResults.find(r => r.subjectId === subject.subjectId);
                return (
                  <div 
